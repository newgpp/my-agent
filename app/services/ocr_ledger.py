...
def split_receipt_entries(lines: List[str]) -> List[List[str]]:
    ...
    list_time_indices = [i for i, line in enumerate(filtered) if LIST_TIME_RE.search(line)]
    if len(list_time_indices) >= 2:
        segments: List[List[str]] = []
        for idx, start in enumerate(list_time_indices):
            end = list_time_indices[idx + 1] if idx + 1 < len(list_time_indices) else len(filtered)
...
def split_receipt_entries_with_bbox(items: List[Dict[str, Any]]) -> List[List[str]]:
    ...
    if len(detail_indices) >= 1:
        ...
        for di in detail_indices:
            segment = [row["text"] for row in rows[start:di] if row["text"] not in NOISE_LINES]
            if segment:
                segments.append(segment)
            start = di + 1
...
async def build_payloads_from_ocr(
